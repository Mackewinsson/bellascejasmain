    ...
    try {
      onSnapshot(doc(db, `Users/${userId}`), (doc) => {
        if (doc.data().estatus && doc.data().estatus == "Activo") {
          ...
          resolve(
            "Estimado usuario, su cuenta ha sido eliminada, contáctenos para más información."
          );
        }
      });
    } catch (error) {
  ...
  try {
    ...
    if (querySnapshot.docs.length > 0) {
      dispatch({
        type: SET_ERRORU,
        payload:
          "Estimado usuario, el correo ingresado ya existe en nuestro sistema",
      });
      ...
      if (user && user.uid) {
        setDoc(doc(db, "Users", user.uid), {
          ...
          .then(function () {
            ...
            axios
              .post(
                "https://microbladingacademy.herokuapp.com/consultas/sendCredentiales",
                JSON.stringify(info),
                { headers: headrs }
              )
              .then(async (response) => {
                if (!response || !response.data || !response.data.success) {
                  dispatch({
                    type: SET_ERRORU,
                    payload: "Error inexperado, contacte con el administrador.",
                  });
                }
                dispatch({ type: LOADING_DATAU, payload: "0" });
              })
              .catch((err) => {
                ...
                dispatch({
                  type: SET_ERRORU,
                  payload:
                    "Estimado usuario, ha ocurrido un error al enviar las credenciales por correo.",
                });
                dispatch({ type: LOADING_DATAU, payload: "0" });
              });
          })
        ...
        dispatch({
          type: SET_ERRORU,
          payload:
            "Error al crear el usuario, intente de nuevo o llame al administrador.",
        });
        dispatch({ type: LOADING_DATAU, payload: "0" });
      }
    }
